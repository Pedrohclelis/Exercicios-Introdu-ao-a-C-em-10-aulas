...
int main(){
	...
	scanf("%s", tamS);
	//Verificação para passar somente números no 'tam'
	while (atoi(tamS) == 0){
		printf("ERRO: '%s' não é um número valido. Digite a quantidade de cadastros desejada: ", tamS);
		scanf("%s", tamS);
	}
	tam = atoi(tamS);
	...
	do{
		...
		printf(">>> Sua resposta: ");
		scanf("%s", cmdS);
		//Verificação para passar somente números no 'cmd'
		while (atoi(cmdS) == 0){
			printf("ERRO: '%s' não é um número valido.\n", cmdS);
			printf(">>> Sua resposta: ");
			scanf("%s", cmdS);
		}
		cmd = atoi(cmdS);
		...
		switch (cmd){
			case 1:
				...
				//Algoritmo pra pegar o mes das compras. Validando somente números inteiros, e que sejam entre 1 a 12
				...
				do{
					...
					switch (mes){
						...
						break;
					default:
						printf("ERRO: '%i' não é um mês valido! Digite um mês entre 1 e 12.\n", mes);
						break;
					}
				} while(mesok == 0);
			...
			case 2:
				...
				//Pega os dados
				printf("===========> BUSCA POR PRODUTO, MÊS, E COMPRADOR <===========\n\n");
			...
			case 3:
				...
				printf("------------------------------------------\n");
				if (total == 0){
					printf("> Desculpe, não há vendas cadastradas para o produto '%s'.\n\n", produto);
				} else{
			...
			case 4:
				...
				printf("------------------------------------------\n");
				if (total == 0){
					printf("> Desculpe, não há compras cadastradas para o cliente de nome '%s'.\n\n", nome);
				} else{
